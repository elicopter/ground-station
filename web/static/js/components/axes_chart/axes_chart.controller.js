function AxesChartController ($scope, blackBoxBrainChannel) {
  var series = [
    [],
    []
    // []
  ...
  var buildGraph = function buildGraph() {
    ...
    y = d3.scaleLinear().domain([40, -40]).range([0, height() - margin.top - margin.bottom]);
  ...
  blackBoxBrainChannel.on("data", function (payload) {
    //  PITCH ROUGE
    // ROLL BLEU
    series[0].push({timestamp: null, value: payload.complementary_axes.roll, type: "roll"});
    series[1].push({timestamp: null, value: payload.complementary_axes.pitch, type: "pitch"});
